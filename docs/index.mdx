...
<Note>
**New in v1.38.0 — Ongoing Component Changelogs in Generated Docs:** `figma_generate_component_doc` now pulls a *real* changelog instead of only echoing hand-written entries. Opt in with `history: { figma: true, git: true }` and the generated doc gains a `## History` section combining **design history** — Figma version history diffed per version and scoped to that component, surfacing renames, added component properties, and token bindings — with **code history** from `git log` on the component's source files, auto-derived from `codeInfo`. Prefers labeled versions and falls back to Figma auto-saves when a file has none (most files don't). Off by default, so existing calls are byte-identical. Server-only release — **no plugin re-import needed**. **Patched in v1.38.1:** the shared-library variable tools (`figma_get_library_variables`, `figma_import_library_variable`) now actually return results — they had silently reported 0 collections since v1.29.0. [See what's new →](https://github.com/southleft/figma-console-mcp/blob/main/CHANGELOG.md#1381---2026-07-29)
</Note>
