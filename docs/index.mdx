...
description: "Your design system as an API. Connect AI to Figma for extraction, creation, debugging, and token sync."
---

<Note>
**New in v1.40.0 (patched v1.40.4) — Design System Extraction:** Turn a production codebase into a design system. Seven Local-Mode `figma_ds_*` tools scan one or more apps (usage-ranked component inventory with vendor classification, variant inference from real call sites, duplicate detection, and a UI-kit-vs-design-system architecture pass), mine the de-facto styling into DTCG tokens with per-token provenance (multi-mode CSS custom properties, SCSS variables, Tailwind config values, shadcn HSL triples, utility-class frequency mining), scaffold the design-system package, wire a fresh Storybook workshop to the app's real theme layers and fonts, deep-extract components for porting with CSF3 story scaffolds, verify with deterministic fidelity evals, and persist porting progress across sessions. The extracted tokens import straight into Figma variables via `figma_import_tokens` — a full code → design system → Figma round-trip. Also fixes the token formatters quoting CSS functional expressions like `cubic-bezier(...)`, which silently killed transitions. Server-only — no plugin re-import needed. [See what's new →](https://github.com/southleft/figma-console-mcp/blob/main/CHANGELOG.md#1400---2026-08-16)
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Codebase → Design System Extraction" icon="dna">
    Turn a production codebase into a design system — analyze, tokenize, scaffold, port, verify. Local Mode only (these tools read and write your local filesystem). The extracted tokens import straight into Figma variables via `figma_import_tokens`.

    - `figma_ds_analyze` — Scan one or more app codebases: framework/styling/vendor detection, usage-ranked component inventory (vendored / wrapped / pure-vendor / bespoke), variant inference from real call sites, duplicate detection, and architecture classification (atomic levels, specializations, missing primitives)
    - `figma_ds_extract_tokens` — Mine styling into DTCG tokens with per-token provenance: multi-mode CSS custom properties, SCSS variables, Tailwind config values, shadcn HSL triples, utility-class frequency mining
    - `figma_ds_scaffold` — Generate the design-system package: token files, token/typography/iconography showcase pages, framework peer deps
    - `figma_ds_setup_storybook` — Wire a fresh Storybook workshop to the extraction: the app's real theme layers and fonts, plus a theme toolbar from the extracted modes
    - `figma_ds_extract_component` — Per-component porting manifest with a ready-to-adapt CSF3 story scaffold
    - `figma_ds_verify` — Deterministic fidelity evals + Figma round-trip readiness
    - `figma_ds_status` — Porting progress persisted across sessions
  </Accordion>
