...
<Note>
**New in v1.35.0 — Figma Slots Support:** Create, inspect, populate, and reset Figma Slots (GA at Config 2026) programmatically. `figma_create_slot` adds slots to components — variants included — with the linked SLOT property created automatically; `figma_append_to_slot` populates instance slots with cloned or newly created content (the path Figma requires — `setProperties` rejects slot values by design); plus `figma_get_slots`, `figma_reset_slot`, and `figma_add_slot_property`. Live-validated against the GA Plugin API. Closes [#29](https://github.com/southleft/figma-console-mcp/issues/29), based on community PR [#77](https://github.com/southleft/figma-console-mcp/pull/77). **Plugin re-import required** (`code.js` + `ui.html` changed). [See what's new →](https://github.com/southleft/figma-console-mcp/blob/main/CHANGELOG.md#1350---2026-07-09)
</Note>
