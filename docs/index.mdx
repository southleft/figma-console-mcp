...
<Note>
**New in v1.30.0 — Native variable binding & typography:** The structured write tools now cover the operations that used to require raw `figma_execute`. `figma_set_fills` / `figma_set_strokes` bind a fill or stroke to a color variable via a new `variableId` param (works on every Figma plan through the bridge — no Enterprise Variables API). `figma_set_text` adds `fontFamily` / `fontStyle` with space-insensitive normalization (`SemiBold` → `Semi Bold`) and graceful fallback. `figma_instantiate_component` pre-loads instance fonts before text overrides (no more silently-skipped overrides) and reports failed overrides in a `warnings` array. **Plugin re-import required** — the bridge files changed. [See what's new →](https://github.com/southleft/figma-console-mcp/blob/main/CHANGELOG.md#1300---2026-06-02)
</Note>
