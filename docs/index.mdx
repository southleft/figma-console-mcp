...
<Note>
**New in v1.34.0 — Bidirectional Token Sync v2 + DTCG 2025.10:** `figma_import_tokens` now applies the *whole* diff plan — creates, renames, alias re-targeting, and replace-gated deletes — for a true code→Figma round-trip. Exports speak DTCG 2025.10 on request (opt-in; the legacy default stays byte-identical), variable scopes/codeSyntax round-trip, `figma_setup_design_tokens` accepts alias values via DTCG brace references, and the new `figma_create_component_set` builds a full variant set from an axes matrix in one call. **Plugin re-import required** (`code.js` + `ui.html` changed). [See what's new →](https://github.com/southleft/figma-console-mcp/blob/main/CHANGELOG.md#1340---2026-07-03)
</Note>
