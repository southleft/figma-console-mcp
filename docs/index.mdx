...
<Note>
**New in v1.31.0 — The "not connected until restart" bug is fixed:** If your Desktop Bridge connection kept dropping and only recovered when you closed the plugin, restarted your MCP client, or killed ports by hand — that was **zombie MCP processes** squatting the WebSocket port range. v1.31.0 force-kills them (`SIGTERM` → `SIGKILL`), sweeps the range every 5 minutes, and adds a backstop so a server can't zombify on shutdown. The plugin now reconnects itself (auto-reconnect watchdog + one-click **Reconnect** button) instead of needing a restart. **Update and re-import the plugin once** to get the fix — then see [Troubleshooting → Start Here](/troubleshooting#connection-drops-or-not-connected-start-here). [See what's new →](https://github.com/southleft/figma-console-mcp/blob/main/CHANGELOG.md#1310---2026-06-05)
</Note>
