...
<Note>
**New in v1.8.0** — WebSocket Bridge transport for CDP-free Figma Desktop communication. Auto-detects and uses WebSocket when available, falls back to CDP. Plus real-time selection tracking, document change events, and live page awareness — all without Chrome DevTools Protocol.
</Note>
...
<Warning>
**Important:** Remote Mode is **read-only** with 16 tools. For design creation, variable management, and full capabilities (53+ tools), use NPX Setup.
</Warning>

<Columns cols={2}>
  <Card title="NPX Setup (Recommended)" icon="bolt" href="/setup#-npx-setup-recommended">
    **Full capabilities — 53+ tools**

    Create designs, manage variables, instantiate components. Requires Node.js and a Figma token. Best for designers who want AI-assisted design creation.
  </Card>
  <Card title="Remote SSE (Read-Only)" icon="cloud" href="/setup#-remote-sse-read-only-exploration">
    **Quick exploration — 16 tools**
...
<Steps>
  ...
  <Step title="Start Figma with Debug Mode">
    Quit Figma, then restart with: `open -a "Figma" --args --remote-debugging-port=9222`
  </Step>
