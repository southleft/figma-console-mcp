...
<Note>
**New in v1.38.2 — Connection Stability Fix:** If your MCP client kept showing **"Server disconnected"**, this release fixes it. The server's own orphan reaper was terminating *healthy* servers: its `/health` liveness probe requested `127.0.0.1` while the server binds `localhost`, which resolves to the **IPv6 loopback** on dual-stack macOS — so the probe reported "nothing responding" for every healthy instance, inverting every kill-safety gate into a rubber stamp. Also fixes non-atomic port-file writes, deletion of port files on a transient read error, and an orphan path that terminated processes without probing them. Affected all MCP clients equally (server-side, unrelated to the Desktop Bridge plugin). **Fully restart your MCP clients after upgrading** — a server still running an older build keeps the broken probe and will terminate healthy siblings. Server-only — **no plugin re-import needed**. [See what's new →](https://github.com/southleft/figma-console-mcp/blob/main/CHANGELOG.md#1382---2026-07-29)
</Note>
