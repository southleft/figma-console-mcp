...
<Note>
**Latest: v1.27.1 (docs patch). New in v1.27.0 — Bidirectional Token Sync:** Two new tools — `figma_export_tokens` and `figma_import_tokens` — replace Style Dictionary and Tokens Studio's export pipeline for popular styling methods. Export Figma variables to canonical DTCG JSON + CSS custom properties; edit hex values in code; push the delta back to Figma. Diff-aware merge means a hacked-color edit produces exactly one Figma API call, not a full collection rewrite. 103 tools total. [See what's new →](/tools#-token-sync-tools)
</Note>
