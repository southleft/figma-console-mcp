...
<Note>
**New in v1.28.0 — Full Formatter Coverage:** `figma_export_tokens` now ships 10 fully-implemented output formats — DTCG JSON, CSS custom properties, Tailwind v4 `@theme inline`, Tailwind v3 config, SCSS, TypeScript module, JSON flat/nested, Style Dictionary v3, and Tokens Studio. Pull Figma variables, emit code in any popular styling flavor — no third-party build tool install required. 103 tools total. [See what's new →](/tools#-token-sync-tools)
</Note>
