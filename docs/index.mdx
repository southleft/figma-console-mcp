...
<Note>
**New in v1.36.0 — Target Lock for Parallel Work:** `figma_navigate` now takes a `lock: true` flag that pins the active file, so an AI agent can keep working in one Figma file while you work in another without commands routing to the wrong file. New connections, reconnects, and your own selection/page changes in other files no longer move the target; the pin auto-releases if that file disconnects or navigates away. `figma_list_open_files` reports a `targetLocked` flag for a pre-write guard. Server-only release — **no plugin re-import needed**. [See what's new →](https://github.com/southleft/figma-console-mcp/blob/main/CHANGELOG.md#1360---2026-07-16)
</Note>
