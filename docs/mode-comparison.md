...
|------|-----------------|--------------|---------------|-----------|
| **Local Mode** (NPX or Git) | **91+** | Yes | Yes | Yes |
| **Cloud Mode** (Remote + Relay) | **43** | Yes | No | No |
| **Remote Mode** (read-only) | **22** | No | No | No |

> **Bottom line:** Remote mode is read-only (81 tools). Cloud Mode adds write access ((81 tools)) without Node.js. Local has everything (91+ tools) including real-time monitoring.

### Use NPX Setup (Recommended for Most Users)
- ✅ **All 91+ tools** including design creation and real-time monitoring
...
- ✅ **(81 tools)** — full write access (create, edit, delete) plus REST API reads
...
- ✅ **All 91+ tools** including design creation
...
- (81 tools): 1 pairing + 27 write tools + 15 REST API reads
...
- All 91+ tools work through WebSocket
...
5. Done ✅ — 81 tools with full write access
...
- ✅ Full 91+ tools including real-time monitoring
...
**Local Mode Transport:** The server automatically selects an available port in the range 9223–9232, supporting multiple simultaneous MCP instances. All 91+ tools work through the WebSocket transport.

**Cloud Mode Transport:** The plugin connects to the Cloudflare relay after pairing. Write operations are relayed from the cloud MCP server through the Durable Object to the plugin. (81 tools) are available.
...
- **Cloud Mode:** (81 tools) — adds full write access (create, edit, delete) via relay
- **Local Mode (NPX/Git):** 91+ tools — adds real-time monitoring (selection, changes, console)
