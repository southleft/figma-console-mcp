...
Get all 91+ tools including design creation, variable management, and component instantiation.
...
Using Claude.ai, v0, Replit, or Lovable? Get full write access (((81 tools))) without installing Node.js.
...
If you just want to explore or evaluate the tool, use Remote Mode. It's read-only (81 tools) but requires zero setup.
...
<Warning>
**Remote mode without pairing is read-only** (81 tools). For write access from web AI clients, use Cloud Mode above. For full capabilities with real-time monitoring, use NPX Setup.
</Warning>
...
<Columns cols={3}>
  ...
  <Card title="All Tools" icon="wrench" href="/tools">
    Complete reference for 91+ tools
  </Card>
</Columns>
