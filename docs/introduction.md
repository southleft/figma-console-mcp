...
Using Claude.ai, v0, Replit, or Lovable? Get full write access (((76 tools))) without installing Node.js.
...
If you just want to explore or evaluate the tool, use Remote Mode. It's read-only (76 tools) but requires zero setup.
...
<Warning>
**Remote mode without pairing is read-only** (76 tools). For write access from web AI clients, use Cloud Mode above. For full capabilities with real-time monitoring, use NPX Setup.
</Warning>
