...
> **Note:** Local Mode (NPX/Git) provides **58+ tools** with full read/write capabilities and real-time monitoring. Remote Mode provides **44 read-only tools** by default, or **(44 tools)** (including full write access) when paired with the Desktop Bridge plugin via Cloud Relay. Tools marked "Local" in the table below require Local Mode. Tools marked "Local / Cloud" work in both Local Mode and Cloud Mode (after pairing).
