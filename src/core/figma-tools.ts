...
import { convertCodeToFigmaSpec, type ComputedStyleMap, type ConversionOptions } from "./code-to-spec.js";
...
export function registerFigmaAPITools(
	...
	// Tool 15: Code to Figma Spec Converter
	server.tool(
		"code_to_figma_spec",
		"Convert rendered HTML + computed CSS styles into a Figma-compatible specification JSON. Use this for Code → Figma workflows where developers want to push UI components from code into Figma as a designer 'sketchpad'. Accepts HTML structure and a map of computed styles (CSS properties → values). Returns a Figma node specification that can be used with the Figma Component Reconstructor plugin.",
		{
			html: z
				.string()
				.describe(
					"HTML string representing the component structure. Should be the rendered DOM output (what the browser sees, not JSX source)."
				),
			styles: z
				.record(
					z.string(), // selector (class name, id, or tag name)
					z.record(z.string(), z.string()) // property → value map
				)
				.describe(
					'Computed CSS styles keyed by selector. Format: { ".button": { "background-color": "#2563eb", "padding": "12px 16px", ... } }'
				),
			componentName: z
				.string()
				.optional()
				.describe(
					'Name for the generated Figma component (e.g., "Button / Primary / Medium")'
				),
			includeMetadata: z
				.boolean()
				.optional()
				.default(false)
				.describe("Include conversion metadata (source info, element count, timing)"),
		},
		async ({ html, styles, componentName, includeMetadata }) => {
			try {
				logger.info(
					{ htmlLength: html.length, styleSelectors: Object.keys(styles).length },
					"Converting code to Figma spec"
				);

				const result = convertCodeToFigmaSpec({
					html,
					styles: styles as ComputedStyleMap,
					options: {
						componentName,
						includeMetadata,
					},
						...
						content: [
							{
								...
								text: JSON.stringify(
									{
										error: result.error,
										message: "Failed to convert code to Figma spec",
										suggestion: "Ensure HTML is valid and styles map contains computed CSS values",
									},
									null,
									2
								),
				...
				logger.info(
					{
						...
						elementsProcessed: result.metadata?.elementsProcessed,
					},
							...
							text: JSON.stringify(
								{
									...
									usage: {
										next_step: "Use this spec with the Figma Component Reconstructor plugin",
										plugin_instructions: "Paste the 'spec' object into the plugin input and click 'Reconstruct'",
									},
								},
								null,
								2
				...
				logger.error({ error }, "Failed to convert code to Figma spec");
				const errorMessage =
					error instanceof Error ? error.message : String(error);
				return {
					content: [
